namespace ClrHeapAllocationAnalyzer
{
    ...
    {
        public static DiagnosticDescriptor ParamsParameterRule = new DiagnosticDescriptor("PAAHeapAnalyzerImplicitParamsRule", "Array allocation for params parameter", "This call site is calling into a function with a 'params' parameter. This results in an array allocation even if no parameter is passed in for the params parameter", "Performance", DiagnosticSeverity.Warning, true);

        public static DiagnosticDescriptor ValueTypeNonOverridenCallRule = new DiagnosticDescriptor("PAAHeapAnalyzerValueTypeNonOverridenCallRule", "Non-overridden virtual method call on value type", "Non-overridden virtual method call on a value type adds a boxing or constrained instruction", "Performance", DiagnosticSeverity.Warning, true);
        ...
        private static void AnalyzeNode(SyntaxNodeAnalysisContext context)
        {
            ...
            IMethodSymbol methodInfo = semanticModel.GetSymbolInfo(invocationExpression, cancellationToken).Symbol as IMethodSymbol;
            if (methodInfo != null)
            {
